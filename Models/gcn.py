...
class GCN(nn.Module):
    def __init__(self, in_features, n_hidden, out_features, noise_dim, dropout):
        ...
        self.gc1_layer_norm = nn.LayerNorm(n_hidden)
        self.gc2 = GraphConvolution(n_hidden, n_hidden)
        self.gc2_layer_norm = nn.LayerNorm(n_hidden)
        self.gc3 = GraphConvolution(n_hidden, n_hidden)
        self.gc3_layer_norm = nn.LayerNorm(n_hidden)
        # try  generative neural radiance fields
        self.fc_noise = nn.Linear(n_hidden, n_hidden - noise_dim)
        self.fc1 = nn.Linear(n_hidden, n_hidden)
        self.fc1_layer_norm = nn.LayerNorm(n_hidden)
        self.fc2 = nn.Linear(n_hidden, n_hidden)
        self.fc2_layer_norm = nn.LayerNorm(n_hidden)
        self.fc3 = nn.Linear(n_hidden, n_hidden)
        self.fc3_layer_norm = nn.LayerNorm(n_hidden)
        self.fc4 = nn.Linear(n_hidden, n_hidden)
        self.fc4_layer_norm = nn.LayerNorm(n_hidden)
        self.fc5 = nn.Linear(n_hidden, n_hidden)
        self.fc5_layer_norm = nn.LayerNorm(n_hidden)
